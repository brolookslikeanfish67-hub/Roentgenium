...
> - Compiler and linker optimizations for performance. Standard desktop releases target [AVX](https://en.wikipedia.org/wiki/Advanced_Vector_Extensions), while compatibility and higher-performance releases use the explicitly named SSE or AVX profiles described in [PATCHES.md](https://github.com/Alex313031/thorium/blob/main/docs/PATCHES.md).
...
> - Linux media, GPU, sandbox, installer, and desktop-integration patches, including VAAPI/libva configuration and Linux packaging behavior tracked in [PATCHES.md](./docs/PATCHES.md).
> - [DoH](https://en.wikipedia.org/wiki/DNS_over_HTTPS) (DNS over HTTPS) defaults and request-header changes adapted from privacy-focused Chromium patchsets.
...
> - Build, compiler, SIMD, and platform packaging patches are applied through the ordered patch series in `patch_scripts/series/series`.
...
> - Includes installer patches and files to include [ChromeDriver](https://chromedriver.chromium.org/home) and *thorium_shell* [(content_shell)](https://chromium.googlesource.com/chromium/src/+/HEAD/docs/testing/web_tests_in_content_shell.md), with a .desktop file being provided for thorium_shell (named thorium-shell.desktop and shown in desktop environments as Thorium Content Shell). You can run content_shell with the command thorium-shell (custom wrapper for it, located in /usr/bin/). You can run ChromeDriver at /usr/bin/chromedriver or chromedriver.exe on Windows. Windows builds also carry the Thorium mini_installer UI patches.
