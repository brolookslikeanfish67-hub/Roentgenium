...
class DownloadShelfUIControllerDelegate : public DownloadUIController::Delegate {
 public:
...
void DownloadShelfUIControllerDelegate::OnNewDownloadReady(
    download::DownloadItem* item) {
  // Here the item will be surfaced to the shelf UI and should
...
#else
    if (download::ShouldShowDownloadBubble(profile)) {
